...
import { LayoutGrid, FolderClosed, FileText, Undo2, Redo2, ShieldAlert, Filter, FilterX, RotateCcw, Columns, Filter as FilterIcon } from 'lucide-react';
...
interface ReusableGridProps<T = any> {
  gridId: string;
  initialRowData?: T[];
  rowData?: T[];
  columnDefs?: ColDef[];
  ...
  transformRowData?: (data: T[]) => any[];
  onRowDataChange?: (newData: T[]) => void;
  treeData?: boolean;
}

export default function ReusableGrid<T extends { id?: string; path?: string[] } = any>({
  gridId,
  initialRowData = [],
  rowData: controlledRowData,
  columnDefs,
  schema = [],
  autoGroupColumnDef: userAutoGroupColumnDef,
  configOverrides = {},
  className = "",
  getDataPath: userGetDataPath,
  ...
  const [internalRowData, setInternalRowData] = useState<T[]>(() => getInitialData());
  
  // Pivot rowData based on controlled/uncontrolled mode
  const rowData = controlledRowData ?? internalRowData;

  const setRowData = useCallback((newData: T[]) => {
    if (onRowDataChange) {
      onRowDataChange(newData);
    }
    if (!controlledRowData) {
      setInternalRowData(newData);
    }
  }, [controlledRowData, onRowDataChange]);
  ...
  const initialDataRef = useRef<T[]>(structuredClone(initialRowData));

  // Persistence
  useEffect(() => {
    if (config.enablePersistence && !controlledRowData) {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(internalRowData));
    }
  }, [internalRowData, config.enablePersistence, STORAGE_KEY, controlledRowData]);
  ...
  const saveExpandedState = useCallback(() => {
    ...
    gridApiRef.current.forEachNode((node) => {
      if (node.expanded && node.data) {
        const id = userGetRowId ? userGetRowId({ data: node.data }) : node.data.id;
        if (id) expandedIds.push(id);
      }
    });
    localStorage.setItem(EXPANDED_KEY, JSON.stringify(expandedIds));
  }, [config.enablePersistence, config.enableExpandedState, EXPANDED_KEY, userGetRowId]);
  ...
  // History
  const updateDataWithHistory = useCallback((newData: T[], isUndoRedo = false) => {
    if (!isUndoRedo && config.enableUndoRedo) {
      setHistory(prev => [...prev, structuredClone(rowData)].slice(-20));
      setFuture([]);
    }
    setRowData(newData);
  }, [rowData, config.enableUndoRedo, setRowData]);
        ...
        try {
          ...
          if (savedExp) {
            ...
            params.api.forEachNode(node => { 
                if (node.data) {
                    const id = userGetRowId ? userGetRowId({ data: node.data }) : node.data.id;
                    if (id && ids.includes(id)) node.setExpanded(true); 
                }
            });
  ...
  const onRowDragEnd = (event: RowDragEndEvent) => {
    ...
    // Check if we are in Task mode for MovementEngine, otherwise user should handle it
    const isTaskData = rowData.length > 0 && 'path' in (rowData[0] as any);
    if (!isTaskData) {
        console.warn("Drag and drop logic in ReusableGrid is currently optimized for path-based Task data. For other structures, please implement custom movement handlers.");
        return;
    }
    ...
    updateDataWithHistory(MovementEngine.moveTasks(rowData as unknown as Task[], movingIds, targetId, position) as unknown as T[]);
  };
  ...
  const handleAddNode = () => {
    if (!gridApiRef.current) return;
    const isTaskData = rowData.length > 0 && 'path' in (rowData[0] as any);
    if (!isTaskData) return;
    
    const parentId = gridApiRef.current.getSelectedRows()[0]?.id || null;
    updateDataWithHistory(MovementEngine.addNode(rowData as unknown as Task[], parentId) as unknown as T[]);
  };
  ...
  const handleDeleteNode = () => {
    ...
    if (targetId) {
      updateDataWithHistory(MovementEngine.deleteNode(rowData as unknown as Task[], targetId) as unknown as T[]);
  ...
  const schemaColumnDefs = useMemo(() => {
...
// ...
  ...
  // Use ColumnDefs directly if provided, otherwise use schema mapper
  const finalColumnDefs = useMemo(() => {
    const baseDefs = columnDefs || schemaColumnDefs;
    
    return baseDefs.map((col, idx) => {
      // Add rowDrag to the first visible column that isn't hidden
      const isFirstColumn = idx === 0; 
      return { 
        ...col, 
        rowDrag: col.rowDrag ?? (isFirstColumn && !isSortActive && config.enableDragDrop) 
      };
    });
  }, [columnDefs, schemaColumnDefs, isSortActive, config.enableDragDrop]);
  ...
  const defaultColDef: ColDef = useMemo(() => ({
    ...
    onCellValueChanged: (params) => {
      ...
      const isTaskData = rowData.length > 0 && 'path' in (rowData[0] as any);
      if (isTaskData) {
        const newData = MovementEngine.updateNode(rowData as unknown as Task[], params.data.id, { ...params.data });
        updateDataWithHistory(newData as unknown as T[]);
      }
    }
  ...
  const processedRowData = useMemo(() => {
    if (transformRowData) {
      return transformRowData(rowData);
    }
    return rowData;
  }, [rowData, transformRowData]);
  ...
  return (
    ...
    >
      ...
      <GridToolbar 
        ...
        onOpenColumns={() => setActivePanel('columns')}
      ...
      <div className="flex-1 bg-[var(--bg-app)] overflow-hidden relative motion-theme">
        <div className="ag-theme-quartz w-full h-full relative motion-theme">
          <AgGridReact
            ...
            getDataPath={userGetDataPath || ((data: any) => data.path)}
            ...
            onRowDragMove={(e) => {
              ...
              if (overNode) {
                  const id = userGetRowId ? userGetRowId({ data: overNode.data }) : overNode.data?.id;
                  if (id) setDropIndicator({ targetId: id, position: getDropPosition(e) });
              }
            }}
            onRowDragLeave={() => setDropIndicator(null)}
            onRowDragEnd={onRowDragEnd}
            getRowId={userGetRowId || ((params) => params.data.id)}
      ...
      <SlideOverPanel 
        ...
        title={activePanel === 'columns' ? 'Manage Columns' : 'Advanced Filters'}
        icon={activePanel === 'columns' ? <Columns size={18} /> : <FilterIcon size={18} />}
      >
        {activePanel === 'columns' ? (
          <ColumnManagement gridApi={gridApi} />
        ) : (
          <div className="flex flex-col items-center justify-center h-64 text-slate-400 gap-4">
            <div className="bg-slate-50 p-6 rounded-full">
              <FilterIcon size={32} className="opacity-20" />
            </div>
            <div className="text-center">
              <h4 className="text-sm font-bold text-slate-600">Filters coming soon</h4>
              <p className="text-xs text-slate-400 mt-1">We're working on advanced hierarchical filtering logic.</p>
            </div>
          </div>
        )}
      </SlideOverPanel>
